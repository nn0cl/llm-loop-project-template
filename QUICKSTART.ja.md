...
- このテンプレート自身の保守用の記録は、導入先リポジトリには入りません。
  copy script がそれらのパスを除外していますし、2026-08-02 以降はテンプレート
  の作業ツリー自体にもそれらは存在しません。それでも兄弟ディレクトリとして
  checkout しておけば、2つの作業ツリーを見比べたときに分離が一目でわかります。
...
したものです。ただしこれはテンプレート v1.0.0 時点の手作業によるスナップショット
...
先に `.github/workflows/ci.yml` の `required_files` リストを削ってください
（後述「削除ではなく編集が必要」参照）。このリストは、この節に挙げた
ファイルを含め、テンプレートが導入するほぼすべてのファイルの存在を
検査しているためです。
...
- `docs/architecture/adr/0001-*.md` から `0014-*.md` までは、このテンプレー
  トが同梱する process ADR です。この 14 件だけを削除し、プロジェクトが
  その後採番した ADR（0015 以降）は残します。
...
- `.github/workflows/ci.yml` ——**最初に**これを編集してください。
  「Check required project documents」step は、上の「そのまま削除して
  よい」節に挙げたスクリプト・テンプレート・process 文書を含む、この
  テンプレートが導入するほぼすべてのファイルの存在を検査し、
  「Check architecture decision records」step は ADR 0001〜0014 を検査
  します。リストを削る前に何かを削除すると、次の push で CI が落ちます。
  実際に残す内容に合わせて step を書き換えるか削除してください。

導入先プロジェクトの `README.md` は、このテンプレートの配布物には
含まれません（テンプレート自身の README はテンプレートリポジトリに
留まります）。したがってアンインストールで触る必要はありません。
`README.md` / `README.ja.md` をコピーしていた旧バージョンのテンプレートで
導入したプロジェクトでは、ファイルを削除するのではなく、テンプレート由来
の記述だけを編集で取り除いてください。

### 配布物だが、それ自体として有用なもの

`.github/dependabot.yml`、`.github/pull_request_template.md`、
`.github/ISSUE_TEMPLATE/` は、このテンプレートの配布物の一部ですが、
その中身は AI 協働プロセスではなく、妥当な既定値を持つ一般的な GitHub
設定です。プロセスを続けるかどうかに関わらず、それ自体として有用なので
残すことをおすすめします。
