...
- `docs/architecture/adr/0001-*.md` から `0017-*.md` までは、このテンプレー
  トが同梱する process ADR です。この 17 件だけを削除し、プロジェクトが
  その後採番した ADR（0018 以降）は残します。
...
- `.github/workflows/ci.yml` ——**最初に**これを編集してください。
  ...
  「Check architecture decision records」step は ADR 0001〜0017 を検査
