...
チャットのウィンドウは成果物ではない。AIセッションで決めたこと、計画したこと、文脈としてエージェントに渡したこと——これらをチャット履歴の中に放っておけば、それは膨大なログに埋もれ、いずれコンテキストウィンドウの彼方へと消え去る。エージェントはセッションの内側では驚異的な自由度で動けるが、それをソフトウェアプロジェクトとして統合するための配線（計画、履歴、承認フロー）は、セッションの外側——すなわちリポジトリ、CI（継続的インテグレーション）、そして人間の承認プロセス——に確固として置かなければならない（[確率的世界との対峙](2026-07-07-rationale-saas-agent-as-sandbox.md)）。
...
読み物の編集のような軽量な作業では、Adjudicator が明示的に issue の作成を省略する（issue なしを認める）こともある。しかしその場合でも、waiver（免除）は口頭の雰囲気で済ませるのではなく、作業報告（trace）に明記されるべきである。issue を作らないことは、手続きを軽量化するという設計判断であって、作業の来歴を完全に消し去るという判断ではない。

## ADR は記録を超え、拘束力のある規約として扱う

アーキテクチャの決定をリポジトリの履歴に置くという発想の原点は、Michael Nygard による ADR（Architecture Decision Records）の提言（[Cognitect](https://cognitect.com/blog/2011/11/15/documenting-architecture-decisions)）に遡る。人間同士のチームであれば、ADR は「決定事項とその背景（Context）をまとめたメモ」として機能したかもしれない。AI エージェントがセッションごとに文脈を失う環境では、**ADR を「過去のメモ」に留めると弱い**。渡された文書を推論の初期条件とするエージェントは、学習済みの一般的ベストプラクティスに引き寄せられ、ローカル決定を破ることがある。

したがって本テンプレートでは、規範側で ADR を **従うべき拘束的な決定**として扱い、契約ファイル（`AGENTS.md` 等）がその執行を現場に下ろし、Issue / Trace が改訂手続きを担う（ADR 0005 / 0006）。これは物理的な強制ではない（[確率的世界との対峙](2026-07-07-rationale-saas-agent-as-sandbox.md)）。Lessig の "Code is Law" は、コードが行動を形作るという有名な類推であり、ここで「リポジトリ規約が生成コードを方向づける」と読む助けにはなるが、**法制度そのものではない**。レビューと CI が支えるのは、比喩上の「法体系」ではなく、文書化された決定と手続きである。

AI 協働では、「どのファイルを読ませたか」「どの phase で止めたか」自体が実装に効く。振る舞いを変えるには契約ファイルを変え、その変更には所定の手続きが要る——古典的な ADR メモだけでは足りない、という実務的な拡張である。

## プロンプトは設定ではなくコードである

`AGENTS.md`、`CLAUDE.md`、`.github/copilot-instructions.md` などのファイルは、エージェントの操作契約を定義している。これらを放置すれば、それぞれが無言で独自の方向へドリフトしていく（ADR 0006）。これらのファイルの変更には明確な理由、Adjudicator によるレビュー、ファイル間の整合性確認、そして trace の記録を要求し、CI が trace の欠落を機械的に拒否する。
...
ADR 0005 が Negative として挙げていたリスク（ローカルファイルと GitHub メタデータのドリフト、ステータス更新が人間の規律に依存しすぎること）は、開発の第2段階での採用者側レビューにおいて「親 issue の陳腐化」という形で現実のものとなった（用語は [README](README.md)）。その実践上の観測は、LISS-0005 という新たな issue へと還流し、プロセス改善のトリガーとなった。
